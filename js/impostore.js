...
function startLocalGame() {
    ...
    // Selezione casuale semplice ma efficace degli indici degli impostori
    let impostorIndices = [];
    while (impostorIndices.length < numImpostors) {
        let randIdx = Math.floor(Math.random() * players.length);
        if (!impostorIndices.includes(randIdx)) {
            impostorIndices.push(randIdx);
        }
    }
...
function revealRoleStep() {
    ...
    let roleContent = "";
    if (isImpostor) {
        let hintHtml = localGameData.showHint ? `
            <div style="margin-top: 15px; padding: 12px; background: rgba(254, 226, 226, 0.8); border-radius: 8px; border: 1px dashed #ef4444;">
                <p style="color: #991b1b; font-size: 0.95rem;">💡 <b>Aiuto Semantico:</b> ${localGameData.selectedWordObj.hint}</p>
            </div>` : "";
...
function nextPlayerOrFinish() {
    ...
    if (localGameData.currentIndex < localGameData.players.length) {
        ...
        gameScreen.innerHTML = `
            ...
            <p style="text-align: center; color: var(--text-muted); margin: 20px 0;">Discutete e fate il vostro giro di indizi dal vivo.</p>
...
function createOnlineRoom() {
    ...
    if (!nickname) { alert("Inserisci un nickname valido!"); return; }
...
function joinOnlineRoom() {
    ...
    if (!nickname || !roomCode) { alert("Inserisci nickname e codice stanza!"); return; }
...
socket.on('impostor_update_players', (players) => {
    ...
    if (isHost && players.length >= 3) {
        let btnStart = document.getElementById('btnStartOnline');
        if (!btnStart) {
            btnStart = document.createElement('button');
            btnStart.id = 'btnStartOnline';
            btnStart.className = 'primary-btn';
            btnStart.style.marginBottom = '10px';
            btnStart.innerText = 'Avvia Partita Online';
            btnStart.onclick = startOnlineGame;
            document.querySelector('#onlineLobbyArea div').appendChild(btnStart);
        }
...
socket.on('impostor_role_assigned', ({ isImpostor, secretWord, players }) => {
    ...
    gameScreen.innerHTML = `
        ...
        <div style="display: flex; flex-direction: column; background: #f8fafc; border: 2px solid var(--border-color); border-radius: var(--radius-md); padding: 10px; margin-bottom: 15px;">
            ...
            <div style="display: flex; gap: 5px;">
                <input type="text" id="chatInput" placeholder="Scrivi il tuo indizio o messaggio..." style="padding: 8px 12px; font-size: 0.9rem; flex: 1;">
    ...
    // Attacca l'evento invio con invio da tastiera sull'input appena creato
...
// Ascoltatore globale sicuro per la chat online
